...
8. Present a ship summary that includes the branch to use or create, any planned commit subject, the push remote, the review remote, the source remote, the provider, the base branch, whether an existing PR or MR will be reused or a new one will be created, and the cleanup steps that will run after a successful merge.
...
14. Squash merge that PR or MR against the same review remote and remove the source branch when the provider supports it.
15. If the provider-aware review flow returns `Blocked` because of checks, approvals, conflicts, permissions, branch protections, or ambiguous review-remote, source-remote, or base-branch resolution, stop before cleanup and report the blocking condition.
...
- Never bypass provider protections with admin-style merge flags.
- Never force push.
