---
name: docs
description: "Write user documentation in Laravel-style prose with reader-first structure, active voice, problem-solution openings, and simple-to-advanced progression. Use when editing `docs/**/*.md` or `**/README.md` files."
---
...
Use callouts sparingly - one or two per major section maximum. Never place two identical callout types consecutively.
...
- Never use callout types other than `[!NOTE]` and `[!IMPORTANT]`
- Never place two identical callout types consecutively (`[!NOTE]` followed immediately by `[!NOTE]`, etc.)
